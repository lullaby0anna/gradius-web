# GD.py
# Single-file local web game server (Canvas 2D). Stage 1 only / Hard / 3 lives.
# Run:  python GD.py
...
<head>
  ...
  <title>ありす的グラディウス（Hard／残機3）</title>
  <style>
...
if (player.laserOn) {
  ...
  function gameOver() {
    ...
    elOverlay.querySelector("p").innerHTML = `Score: <b>${score}</b><br/>Press <span class="kbd">R</span> to restart.`;
  ...
  function gameWin() {
    ...
    elOverlay.querySelector("p").innerHTML = `Score: <b>${score}</b><br/>ステージ1クリア。<span class="kbd">R</span>でリスタート。`;
    elStart.textContent = "リスタート";
    elStart.style.display = "inline-block";
    elHow.style.display = "none";
    elHowTxt.style.display = "none";
...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            r = _Resp(HTML.encode("utf-8"), "text/html; charset=utf-8")
        ...
        # serve local BGM files
        p = self.path.split("?", 1)[0]
        if p in ("/geme.mp3", "/boss2.mp3", "/boss.mp3"):
            from pathlib import Path
            fn = p.lstrip("/")
            fpath = Path(__file__).with_name(fn)
            if (not fpath.exists()):
